...
def model_fn(features, labels, mode, params):
    ...
    # TODO: If its possible to run the mesh in multi-node setup
    # First we need to create a cluster spec
    # Clusterspec should create TF servers on machines
    # Then it is mapped to Tensorflow devices
    # Also we might need to create a new strategy like "MeshTensorFlowStrategy" similar to "ParameterServerStrategy"
    # The TF devices will look something like this
    # "/job:{job-type}/task:{task-index}/device:{hostname:port}"
    #
    # mesh_devices = ["/job:worker1/task:1/device:CPU:0",
    #                 "/job:worker2/task:1/device:CPU:0",
    #                 "/job:worker3/task:2/device:CPU:0",
    #                 "/job:worker4/task:3/device:CPU:0"]
